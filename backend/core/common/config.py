"""Typed application configuration loaded from ``.env`` (pydantic-settings).

Every key in ``.env.example`` maps to a field here. ``OLLAMA_API_KEY`` is the
only real secret (``SecretStr``); everything else is non-sensitive local config.
Defaults let the app run even with no ``.env`` present (e.g. CI), in which case
``LLM_MODE`` defaults to ``fallback`` — the deterministic offline reasoner
("mock") — so no key/network is required until a cloud provider is selected.
"""

from __future__ import annotations

from functools import lru_cache
from pathlib import Path
from typing import Literal

from pydantic import SecretStr
from pydantic_settings import BaseSettings, SettingsConfigDict

# Repo root = three levels up from this file: backend/core/common/config.py
REPO_ROOT = Path(__file__).resolve().parents[3]


class Settings(BaseSettings):
    """Application settings. Field names match ``.env`` keys (case-insensitive)."""

    model_config = SettingsConfigDict(
        env_file=REPO_ROOT / ".env",
        env_file_encoding="utf-8",
        extra="ignore",
        case_sensitive=False,
    )

    # --- LLM behaviour ---
    # Default is the deterministic offline reasoner ("mock", i.e. fallback): the
    # app runs with no key/network out of the box; switch to a cloud provider in
    # the UI (or via env) for live reasoning.
    llm_mode: Literal["cloud", "fallback"] = "fallback"
    # Which cloud provider serves the reasoning nodes when llm_mode="cloud".
    llm_provider: Literal["ollama", "azure_openai"] = "ollama"
    llm_fallback_enabled: bool = True
    llm_timeout_seconds: int = 150
    llm_max_retries: int = 2
    llm_retry_backoff_seconds: float = 2.0

    # --- Ollama Cloud API (a real secret) ---
    ollama_host: str = "https://ollama.com/v1"
    ollama_model: str = "glm-5.2"
    ollama_api_key: SecretStr = SecretStr("")

    # --- Azure OpenAI (a real secret) ---
    # Used when LLM_PROVIDER=azure_openai. The deployment name (not a model id)
    # goes in the request URL; auth is the "api-key" header.
    azure_openai_endpoint: str = ""
    azure_openai_deployment: str = ""
    azure_openai_api_version: str = "2024-10-21"
    azure_openai_api_key: SecretStr = SecretStr("")

    # --- Backend API ---
    api_host: str = "127.0.0.1"
    api_port: int = 8000

    # --- Frontend (Gradio) ---
    gradio_host: str = "127.0.0.1"
    gradio_port: int = 7860
    backend_base_url: str = "http://127.0.0.1:8000"

    # --- Local runtime storage ---
    data_dir: Path = REPO_ROOT / "data"
    runtime_input_dir: Path = REPO_ROOT / "data" / "runtime" / "input"
    runtime_processing_dir: Path = REPO_ROOT / "data" / "runtime" / "processing"
    runtime_operations_dir: Path = REPO_ROOT / "data" / "runtime" / "operations"
    runtime_approvals_dir: Path = REPO_ROOT / "data" / "runtime" / "approvals"
    runtime_completed_dir: Path = REPO_ROOT / "data" / "runtime" / "completed"
    runtime_logs_dir: Path = REPO_ROOT / "data" / "runtime" / "logs"
    hiring_tracker_path: Path = REPO_ROOT / "data" / "hiring_tracker.json"

    # --- File Watcher ---
    # When true, the FastAPI app starts the Watchdog File Watcher on the input
    # directory at startup. Disabled by tests via the create_app factory.
    enable_file_watcher: bool = True

    # --- Mock execution ---
    # Name of an adapter ("ats" | "email" | "teams") to force-fail, so the
    # execution-failure path can be exercised. Empty = all adapters succeed.
    mock_execution_fail_adapter: str = ""

    # --- Logging ---
    log_level: str = "INFO"

    @property
    def runtime_dirs(self) -> list[Path]:
        """All runtime directories the File Watcher / session manager use."""
        return [
            self.runtime_input_dir,
            self.runtime_processing_dir,
            self.runtime_operations_dir,
            self.runtime_approvals_dir,
            self.runtime_completed_dir,
            self.runtime_logs_dir,
        ]

    @property
    def has_ollama_key(self) -> bool:
        return bool(self.ollama_api_key.get_secret_value())

    @property
    def has_azure_key(self) -> bool:
        return bool(self.azure_openai_api_key.get_secret_value())


@lru_cache
def get_settings() -> Settings:
    """Return a cached ``Settings`` instance."""
    return Settings()
